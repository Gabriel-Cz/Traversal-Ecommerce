...
import { AddToCartButton, CheckoutButton } from '../Buttons'
    ...
    return(
        <>
          <div className="d-flex justify-content-center align-items-center ">
              <SvgTruck></SvgTruck>
              <small className="ml-4 text-muted" styles={{font: "'Lora', serif"}}>Shipment from 3 to 5 days</small>
          </div>
        </>
    );
}

export default function ProductInformation({title, material, description, rating}) {
    return(
        <>
          <div className="h-100">
              <div className="mt-3 px-4">
                  <h2 className={styles.productName}>{title}</h2>
                  <p className="my-3 text-muted"><i>{description}</i></p>
